...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if(gEv->ComputeElectron()){//Check hor spectro accep.
    ...
    G4double tmax = std::max(t_cos_plus, t_cos_minus);
    G4double tmin = std::min(t_cos_plus, t_cos_minus);
    ...
    if(gEv->HitsSpectro(gEv->GetScatteredElectron()) && (gEv->HitsCalo(gEv->GetFinalPhoton1()) && gEv->HitsCalo(gEv->GetFinalPhoton2()))){//Checks calo acceptance
        ...
        if(gEv->HitsCalo(gEv->GetFinalPhoton1())) L_phot1_final = gEv->GetFinalPhoton1(); 
        if(gEv->HitsCalo(gEv->GetFinalPhoton2())) L_phot2_final = gEv->GetFinalPhoton2(); 
        ...
        if(hit_HMS){
	        ...
	        fEventAction->DefinePrimaries(L_elec_init, L_elec_scat, L_phot1_final, L_phot2_final, L_prot_final, gEv->GetVertex()->X(), gEv->GetVertex()->Y(), gEv->GetVertex()->Z());
            ...
            // if(gEv->HitsCalo(gEv->GetFinalPhoton1())){
            ...
            // if(gEv->HitsCalo(gEv->GetFinalPhoton2())){
                fParticleGun->SetParticleDefinition(particleTable->FindParticle(22));//22 : photon
